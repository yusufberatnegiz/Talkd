alter table public.profiles
  add column if not exists listen_back_required integer not null default 0;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'profiles_listen_back_required_nonnegative'
  ) then
    alter table public.profiles
      add constraint profiles_listen_back_required_nonnegative check (listen_back_required >= 0);
  end if;
end $$;

create or replace function public.get_listen_back_status()
returns table (
  listens_required integer,
  can_talk boolean
)
language plpgsql
security definer
...
begin
  ...
  return query
  select
    coalesce(p.listen_back_required, 0) as listens_required,
    coalesce(p.listen_back_required, 0) = 0 as can_talk
...
begin
  ...
  if session_row.participant_a is not null then
    update public.profiles
    set listen_back_required = case
        when session_row.intent_a in ('listen', 'listener') then greatest(0, listen_back_required - 1)
        else listen_back_required + 2
      end
    where id = session_row.participant_a;
  end if;

  if session_row.participant_b is not null then
    update public.profiles
    set listen_back_required = case
        when session_row.intent_b in ('listen', 'listener') then greatest(0, listen_back_required - 1)
        else listen_back_required + 2
      end
    where id = session_row.participant_b;
  end if;
end;
$$;

drop function if exists public.find_or_create_match(text, text, text, text, boolean, boolean);
...
begin
  ...
  if p_role = 'talker'
    and not p_skip_listen_back_gate
    and exists (
      select 1
      from public.profiles p
      where p.id = caller_id
        and coalesce(p.listen_back_required, 0) > 0
    )
  then
    raise exception 'Listen-back required before starting another talk.';
  end if;
  ...
  left join lateral (
    select
      ...
      count(*) filter (where r.badge in ('listener', 'supportive', 'present')) as helpful_count,
      max(r.created_at) filter (where r.badge in ('listener', 'supportive', 'present') or r.stars >= 4) as last_helpful_at
    from public.session_ratings r
    where r.rated_user_id = q.user_id
  ) listener_quality on true
  ...
  order by
    ...
    case
      when p_priority_listener_preference and p_role = 'talker' and q.role = 'listener'
        and coalesce(listener_quality.avg_stars, 0) >= 4
        and coalesce(listener_quality.helpful_count, 0) > 0
      then 0
      else 1
    end,
    case
      when p_priority_listener_preference and p_role = 'talker' and q.role = 'listener'
      then coalesce(listener_quality.avg_stars, 0)
      else 0
    end desc,
    case
      when p_priority_listener_preference and p_role = 'talker' and q.role = 'listener'
      then coalesce(listener_quality.helpful_count, 0)
      else 0
    end desc,
    case
      when p_priority_listener_preference and p_role = 'talker' and q.role = 'listener'
      then coalesce(extract(epoch from listener_quality.last_helpful_at), 0)
      else 0
    end desc,
    q.created_at asc
...
revoke all on function public.get_listen_back_status() from public;
revoke all on function public.get_listen_back_status() from anon;
grant execute on function public.get_listen_back_status() to authenticated;
...
revoke all on function public.find_or_create_match(text, text, text, text, boolean, boolean, boolean) from public;
revoke all on function public.find_or_create_match(text, text, text, text, boolean, boolean, boolean) from anon;
grant execute on function public.find_or_create_match(text, text, text, text, boolean, boolean, boolean) to authenticated;
